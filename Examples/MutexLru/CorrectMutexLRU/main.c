...
void* set_high_water_mark(void* arg) {
    int rv;
    ...
    // only HIGH_WATER_MARK value.
    print();
    return NULL;
}

void self_tests_ex2_1() {
    pthread_t t1;
    pthread_create(&t1, NULL, set_high_water_mark, NULL);
    sleep(1);
    // clean, to make reference not to block at HIGH_WATER_MARK
    clean(0);
    pthread_join(t1, NULL);
    clean(0);
    print();
    printf("end self_tests_ex2_1\n");
}

void* set_low_water_mark(void* arg) {
    int rv;
    ...
    print();
    printf("block at LOW_WATER_MARK\n");
    // block at LOW_WATER_MARK.
    clean(1);
    print();
...
void self_tests_ex2_2() {
    pthread_t t1;
    pthread_create(&t1, NULL, set_low_water_mark, NULL);
    sleep(1);
    // break the LOW_WATER_MARK
    reference(LOW_WATER_MARK);
    pthread_join(t1, NULL);
    printf("end self_tests_ex2_2\n");
}
